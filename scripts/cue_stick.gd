...
func start_aiming(ball: RigidBody2D):
    ...
    # Position cue stick tip at mouse position initially
    var mouse_pos = get_global_mouse_position()
    var ball_pos = ball.global_position
    var initial_direction = (mouse_pos - ball_pos).normalized()
    
    # Position cue stick tip exactly at mouse position
    var cue_length = 100.0  # Approximate length of cue stick sprite
    var tip_to_center_offset = initial_direction * (cue_length / 2)
    global_position = mouse_pos - tip_to_center_offset
...
func update_aim(mouse_pos: Vector2):
    ...
    # Position cue stick tip exactly at mouse position
    # The sprite's pivot is at center, so offset by half the cue length
    var cue_length = 100.0  # Approximate length of cue stick sprite
    var tip_to_center_offset = mouse_direction * (cue_length / 2)
    global_position = mouse_pos - tip_to_center_offset
    
    # Point cue stick toward the ball
...
func cpu_show_aim(ball: RigidBody2D, direction: Vector2, power_ratio: float):
    ...
    # Position cue stick so tip points at ball (since sprite is flipped)
    var cue_length = 100.0  # Approximate length of cue stick sprite
    var stick_offset = direction * (distance + cue_length/2)  # Position tip at proper distance
...
func cpu_thinking_animation(ball: RigidBody2D, base_direction: Vector2, power_ratio: float):
    ...
    var elapsed = 0.0
    var dt = 1.0 / 60.0  # Assume 60 FPS for smooth animation
    ...
    while elapsed < thinking_time:
        ...
        # Position cue stick so tip points at ball (since sprite is flipped)
        var cue_length = 100.0
        var stick_offset = adjusted_direction * (distance + cue_length/2)
        global_position = ball.global_position + stick_offset
        rotation = adjusted_direction.angle() + PI
